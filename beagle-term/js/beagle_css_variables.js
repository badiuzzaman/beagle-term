 ...
 * color scheme.
 */
Beagle.CSSVariables = function(styleSheet) {
 ...
 */
Beagle.CSSVariables.reFindVar = /var\s*\(\s*([^\)]+)\s*\)/;


/**
 * Regexp to match variable references.
 */
Beagle.CSSVariables.reReplaceVar = /var\s*\(\s*([^\)]+)\s*\)/g;
 ...
 */
Beagle.CSSVariables.prototype.parse_ = function(source) {
  ...
  while (pos < source.length) {
    ...
    ruleText.split(';').forEach(function(pair) {
        ...
        if (Beagle.CSSVariables.reFindVar.test(value)) {
 ...
 */
Beagle.CSSVariables.prototype.loadSource_ = function() {
 ...
 */
Beagle.CSSVariables.prototype.sync = function() {
  ...
  this.dynamicRules_.forEach(function(ary) {
      ...
      var prop = ary[1];
      var value = ary[2];
      value = value.replace(Beagle.CSSVariables.reReplaceVar, replaceVar);
 ...
 * this.sync().
 */
Beagle.CSSVariables.prototype.reset = function(opt_map) {
 ...
 */
Beagle.CSSVariables.prototype.set = function(var_args) {
